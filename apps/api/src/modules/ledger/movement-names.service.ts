...
export class MovementNamesService {
  ...
  async attachNames(records: readonly MovementRecord[]): Promise<MovementWithNames[]> {
    ...
    return records.map((record) => {
      ...
      const returnedBy = record.returnedByWorkerId ? workersById.get(record.returnedByWorkerId) : undefined;
      ...
      return {
        ...
        keeper: keeper ? toKeeper(keeper) : { keeperId: record.keeperId, fullName: record.keeperId },
      };
    });
  }
}
